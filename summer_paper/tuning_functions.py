...
Centralising tuning functions => modified for summer paper
    ...
    for n, nrun in enumerate(T_S_profile.nemo_run):
        ...
        for kisf in nisf_list:
            ...
            melt_rate_2D_isf = meltf.calculate_melt_rate_2D_1isf(kisf, filled_TS, geometry_info_2D.sel(nemo_run=nrun), 
                                                                 ...
                                                                 C=C, E0=None, angle_option='local', 
...
def load_data_nemo_tuning_summer_paper(run_list):
    file_isf_list = []
    file_other_list = []
    file_conc_list = []
    file_TS_list = []
    ...
    for n,nemo_run in enumerate(run_list):
        ...
        if nemo_run in ['ctrl94','isf94','isfru94']:
            ...
            inputpath_profiles = '/bettik/burgardc/SCRIPTS/basal_melt_param/data/interim/T_S_PROF/nemo_5km_'+nemo_run+'/'
            file_TS_orig = xr.open_dataset(inputpath_profiles+'T_S_mean_prof_corrected_km_contshelf_and_offshore_1980-2018_oneFRIS.nc').sel(profile_domain=50).squeeze().drop('profile_domain')
        file_TS = file_TS_orig.sel(Nisf=nonnan_Nisf)
        if nemo_run in ['OPM031','isf94','isfru94']:
            file_TS = file_TS.assign_coords({'time': np.arange(1,len(file_TS.time)+1)+n*100})
        else:
        ...
        target_melt_Gt_yr = NEMO_melt_rates_1D['melt_Gt_per_y_tot'].sel(Nisf=file_isf.Nisf)
        if nemo_run in ['OPM031','isf94','isfru94']:
            target_melt_Gt_yr = target_melt_Gt_yr.assign_coords({'time': np.arange(1,len(target_melt_Gt_yr.time)+1)+n*100})
        else:
            target_melt_Gt_yr = target_melt_Gt_yr.assign_coords({'time': np.arange(1,len(target_melt_Gt_yr.time)+1)+n*50})
        target_melt_list.append(target_melt_Gt_yr)
    ...
    if (option_param == 'plume') or (picop_opt == 'yes'):
        ...
        con1 = {'type':'ineq','fun': PICO_constraints, 
                'args': (random_nisf_sample, filled_TS, geometry_info_2D, geometry_info_1D, 
                         isf_stack_mask, mparam, 'local', box_2D_all, box_1D_all, nD_config, 'nD_config', 
...
def tuning_param_BT(option_param, tuning_approach, dom, 
    ...
    if 'profile_domain' in file_TS_all.dims:
        filled_TS = file_TS_all.sel(profile_domain=dom).squeeze().drop('profile_domain')
    else:
        filled_TS = file_TS_all
